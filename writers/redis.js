...
// collection → royalty basis points cache ('-' = unknown). circuit:nft:royalty:{collection}
async function cacheCollRoyalty(collection, bps) {
  const r = await getClient();
  if (!r) return;
  try { await r.setex(`circuit:nft:royalty:${collection}`, NFT_MINT_COLL_TTL, bps == null ? '-' : String(bps)); } catch {}
}

async function getCachedCollRoyalties(collections) {
  const out = new Map();
  const r = await getClient();
  if (!r) { collections.forEach((c) => out.set(c, undefined)); return out; }
  try {
    ...
    for (let i = 0; i < collections.length; i += CHUNK) {
      const slice = collections.slice(i, i + CHUNK);
      const vals = await r.mget(...slice.map((c) => `circuit:nft:royalty:${c}`));
      slice.forEach((c, j) => out.set(c, vals[j] === null ? undefined : vals[j]));
    }
  } catch { collections.forEach((c) => { if (!out.has(c)) out.set(c, undefined); }); }
  return out;
...
module.exports = {
  ...
  cacheCollName, getCachedCollNames, cacheCollRoyalty, getCachedCollRoyalties,
  disconnect,
};
