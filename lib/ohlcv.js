...
class OHLCVAggregator {
   ...
   * @param {number}  price      - price in SOL or USD
   * @param {number}  volume     - trade volume for this tick (SOL or USD)
   * @param {number}  [ts]       - unix ms timestamp (default: now)
   * @param {boolean} [isBuy]    - true = buy, false = sell, undefined = unknown
   */
  tick(mint, price, volume = 0, ts = Date.now(), isBuy = undefined) {
    for (const w of WINDOWS) {
      ...
      if (!bucket) {
        ...
        bucket.ticks++;
        if (isBuy === true)  bucket.buys++;
        if (isBuy === false) bucket.sells++;
        bucket.lastTick = ts;
      }
    }
  }
  ...
  /**
   * Snapshot every open bucket to the writers and evict only the dead ones.
   *
   * Each call emits all open buckets (a live, idempotent snapshot — the candle store
   * dedupes by openTime), then deletes only buckets with no tick for `maxAgeMs`
   * (genuinely dead tokens — the original memory-growth guard). Active long-window
   * buckets (1h/1d) are kept so they accumulate over their full period instead of
   * being reset every cycle, which previously produced duplicate, partial candles.
   * Emissions are now cumulative snapshots, so the writers coalesce by max/latest, not
   * sum (see writers/redis.js Lua + writers/postgres.js upsert).
  ...
  evictStale(maxAgeMs = 2 * 3_600_000) {
    ...
    for (const map of this._buckets.values()) {
      for (const [key, bucket] of map.entries()) {
        try { this._onCandle(bucket); } catch {}                 // live snapshot (idempotent)
        if (now - (bucket.lastTick ?? bucket.openTime) > maxAgeMs) {
          map.delete(key);                                       // evict only if dead
          evicted++;
        }
      }
    }
