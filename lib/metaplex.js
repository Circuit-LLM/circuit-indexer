...
// Batched collection-mint → name resolver (the collection is itself an NFT with metadata).
// Returns Map(collection → name|null). Off-Helius (Triton), cache the result forever.
async function resolveNames(collections, { chunkSize = 100 } = {}) {
  const out = new Map();
  const pdas = collections.map((c) => ({ collection: c, pda: metadataPda(c) }));
  for (let i = 0; i < pdas.length; i += chunkSize) {
    const slice = pdas.slice(i, i + chunkSize);
    const addrs = slice.map((s) => s.pda).filter(Boolean);
    if (!addrs.length) { slice.forEach((s) => out.set(s.collection, null)); continue; }
    let res = null;
    try { res = await rpcCall('getMultipleAccounts', [addrs, { encoding: 'base64', dataSlice: { offset: 0, length: 120 } }]); }
    catch { res = null; }
    const values = res?.value ?? [];
    let vi = 0;
    for (const s of slice) {
      if (!s.pda) { out.set(s.collection, null); continue; }
      const acc = values[vi++];
      out.set(s.collection, acc?.data?.[0] ? parseName(Buffer.from(acc.data[0], 'base64')) : null);
    }
  }
  return out;
}

module.exports = { metadataPda, parseCollectionKey, parseName, resolveCollections, resolveNames, METAPLEX };
