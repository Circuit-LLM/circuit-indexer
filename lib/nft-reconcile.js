...
async function reconcile(opts = {}) {
  ...
  try {
    ...
    // expire removed (in snapshot last cycle, gone now). A removed listing = FILLED (sold) or DELISTED;
    // record it as a recent-activity event at its last list price before deleting (broad, RPC-free —
    // the token-balance tx approach misses cNFT/bid-fill volume, so this reconciliation diff is what we use).
    const gone = [..._prevAssetIds].filter(id => !current.has(id));
    let recorded = 0;
    if (gone.length) {
      try {
        const [goneRecs, goneColls] = await Promise.all([
          redis.getNftListingsBatch(gone),
          redis.getCachedMintCollections(gone),
        ]);
        for (const id of gone) {
          const rec = goneRecs.get(id);
          const coll = goneColls.get(id);
          if (rec && rec.priceSol > 0 && coll && coll !== '-') {
            await redis.writeNftSale(coll, { assetId: id, priceSol: rec.priceSol, seller: rec.seller, ts: Date.now(), type: 'removed' });
            recorded++;
          }
        }
      } catch (e) { Logger.warn('nft-reconcile: removal capture failed', { error: e.message }); }
    }
    ...
    try {
      ...
      const [cachedNames, cachedRoy] = await Promise.all([
        redis.getCachedCollNames(collList),
        redis.getCachedCollRoyalties(collList),
      ]);
      // Resolve a sample NFT's metadata for any collection missing EITHER its name or its royalty.
      const needMeta = collList.filter((c) => cachedNames.get(c) === undefined || cachedRoy.get(c) === undefined)
        .slice(0, MAX_RESOLVE_PER_CYCLE);
      if (needMeta.length) {
        const sampleOf = new Map();                        // sampleAssetId → collection
        for (const c of needMeta) { const items = collectionMap.get(c); if (items && items.length) sampleOf.set(items[0].assetId, c); }
        const meta = await metaplex.resolveMeta([...sampleOf.keys()]);   // assetId → { name, royaltyBps }
        for (const [assetId, m] of meta) {
          ...
          const collName = m.name ? m.name.replace(/[\s#_\-–—]+\d+\s*$/, '').trim() : null;
          await redis.cacheCollName(collection, collName || null);
          await redis.cacheCollRoyalty(collection, m.royaltyBps);
        }
        namesResolved = needMeta.length;
      }
    } catch (e) { Logger.warn('nft-reconcile: metadata resolution failed', { error: e.message }); }
